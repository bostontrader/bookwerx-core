//const defineRoutes = function (server, mongoDb) {
  ...
  * A list of account numbers.
  ...
  server.get('/lint2', (req, res, next) => {
  mongoDb.collection('distributions').aggregate([
  {$group: {_id: {transaction_id:'$transaction_id', currency_id:'$currency_id'}, t: {$sum: '$amount'}, c: {$sum: 1}}},
  {$lookup: {
  from: 'transactions',
  localField: '_id.transaction_id',
  foreignField: '_id',
  as: 'transactions'
  }},
  {$unwind: '$transactions'},
  //{$project: {_id: false, amount: true, currency_id: true, yy: {$year: '$transaction_info.datetime'}, mm: {$month: '$transaction_info.datetime'}}},
  {$project: {_id: false, transaction_id:'$transactions._id', curex:'$transactions.curex', t: true}},

  {$match: {curex: {$ne: "1"}}},
  //{$group: {_id: {transaction_id:'$_id.transaction_id'}, c: {$sum: 1}}},
  {$sort: {t: 1}}
  ...
  res.setHeader('Access-Control-Allow-Headers', 'X-Requested-With')
  res.json(result.filter(value => {
  return Math.abs(value.t) > 10e-12
  //return Math.abs(value.t) > 0
  //return true
  }))
  })
  .catch(error => {
